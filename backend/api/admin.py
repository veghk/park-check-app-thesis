...
_AI_DIR           = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', '..', 'ai'))
...
class TestResultAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        custom = [
            path(
                ...
                name='api_testresult_changelist',
            ),
            path(
                'img/<str:folder>/<str:filename>/',
                self.admin_site.admin_view(self.serve_image),
                name='api_testresult_img',
            ),
        ]
    ...
    def serve_image(self, request, folder, filename):
        ...
        if not filename.endswith('.jpg') or '/' in filename or '..' in filename:
            raise Http404
        img_path = (
            os.path.join(_TEST_PLATES_DIR, filename) if folder == 'orig'
            else os.path.join(_RESULTS_ROOT, folder, filename)
        )
